...
function [bt, b_post, Pt, ft_tlag]  = forward_filtering_backward_sampler(Yt, Ht, At, Xt, R, mt, Ft, Gt, K, Q, b0, P0, ndraws) 
...
for i = 1:ndraws
    ...
    bt_path(i, nobs, :) = mvnrnd(zeros(size(bT_T)),eye(size(PT_T,1)))*chol(PT_T)+bT_T;
    
    for j = 0:nobs-2
        % derive first the mean bt_t+1 and variance Pt_t+1 for the draws of bt for
        % t = T-1, ... , 1
        part1 = reshape(Pt_t(nobs-1-j, :, :), nstates, nstates) *Ft';
        part2 = eye(nstates)/(Ft*reshape(Pt_t(nobs-1-j, :, :), nstates, nstates)*Ft'+diag(K(nobs-1-j,:))*Q);
        ...
        part5    =  part2;
        part6    =  Ft*reshape(Pt_t(nobs-1-j,:,:), nstates, nstates);
        Pt_tpl1  =  reshape(Pt_t(nobs-1-j,:,:), nstates, nstates) - part4*part5*part6;
